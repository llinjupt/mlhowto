...
.. figure:: imgs/practice/nnproj.jpg
  :scale: 80%
  :align: center
  :alt: nnproj
...
这个现象被称作是梯度消失问题（vanishing gradient problem）。可以想象如果链式中的 w 项大于 4，那么 w 可以中和导数带来的缩小，但是这又会导致梯度激增问题（exploding
...
.. code-block:: python
  ...
  def sklearn_mnist(ratio=1.0, hidden_neurons=10, alpha=0):
      from sklearn.neural_network import MLPClassifier
